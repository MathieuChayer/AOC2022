...
# Create World Map
y = np.arange(min_y-1,max_y+3,1,dtype=int)
x = np.arange(min_x-len(y),max_x+len(y),1,dtype=int)
...
Z[np.where(y==max_y)[0]+2,:] = 1

count_blocks = 0
while not Z[np.where(y==sand_source[1]),np.where(x==sand_source[0])]:
    sandblock()
    count_blocks+=1

print("Blocks before blocked :", count_blocks)
